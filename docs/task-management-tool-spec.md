...
- 使用中のステータスは削除不可（アーカイブのみ）とし、データ不整合を防ぐ
...
- Firestore Security Rules でロールベースの書き込み制御を実装
...
```
{
  ...
  isDefault: boolean
  allowedNextStatuses: string[] | null  // 将来のフロー制御用
}
```
...
- **子課題の `projectId` と `workflowTypeId` は必ず親課題と同一とする。** アプリケーション側のバリデーションおよび可能であれば Firestore Security Rules（`get()` で親ドキュメントを参照・比較）で整合性を担保する
...
- コールドスタート時の処理時間を考慮し、Cloud Functionsのメモリ割り当て・タイムアウト設定は標準よりやや余裕を持たせる（目安：メモリ512MB程度）
...
以下のTypeScript/React・Cloud Functionsコードとして実装済み（本仕様書と対の設計成果物）。

| ファイル | 内容 |
|---|---|
| `types.ts` | `Project` / `WorkflowType` / `Issue` / `Status` / `User` / `Priority` 等のドメイン型（`searchTokens`/`searchReading`含む） |
| `dueDateAlert.ts` | 期限超過・期限接近の判定ロジック |
| `sortAndFilter.ts` | プロジェクト/種別/担当者/ステータス/優先度フィルタ、親子ツリー構築（兄弟間ソートで隣接関係を維持、種別をまたぐステータス比較は`progressPercent`にフォールバック） |
| `kanaUtils.ts` | カタカナ→ひらがな正規化ユーティリティ（クライアント・サーバー共用） |
| `useIssueSearch.ts` | Fuse.js による全文検索フック（生テキスト用・読み用の2インデックスをマージ） |
| `functions-src/tokenizeIssue.ts` | Cloud Functions（課題書き込み時にkuromoji.jsでトークン化・読み正規化 |
| `IssueRow.tsx` | 1行分の表示コンポーネント（プロジェクト・種別バッジ含む） |
| `IssueListView.tsx` | プロジェクト/種別/検索/フィルタ（ステータスは種別ごとにグルーピング表示）/テーブル/ページネーションを統合したメインコンポーネント |

Firestoreからのデータ取得ロジック（`onSnapshot` 等）およびプロジェクト管理画面（作成・編集・アーカイブ）は未実装で、次のステップに含める。

> 実装メモ：`functions-src/tokenizeIssue.ts` は `kanaUtils.ts` を `../../shared/kanaUtils` からimportする想定で書かれており、実プロジェクトのディレクトリ構成（Cloud Functions側とフロントエンド側でのモジュール属方法）に応じて調整すること。

## 8. 未確定事項（次のステップで詰める）

- 種別マスタ（`workflowTypes`）・種別ごとのステータスマスタの管理画面（作成・編集・並び替え）の仕様
- プロジェクト管理画面（作成・編集・アーカイブ）の仕様
- 子課題の `projectId`・`workflowTypeId` 整合性チェックを Firestore Security Rules でどこまで強制するか（`get()` によるドキュメント参照コストとのトレードオフ）
- カンバンビューでのドラッグ&ドロップによるステータス変更をいつのタイミングで追加するか
- 通知機能を追加する際の実装方式（FCM／プッシュ／メール／両方）
- Firestoreデータ取得層（`onSnapshot` によるリアルタイム同期 or 都度フェッチ）の方式
- Firestore Security Rules の具体的なルール定義
- Cloud Functionsのメモリ・タイムアウト設定の最終値（実測ベースでのチューニング）
