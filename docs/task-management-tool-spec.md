...
| 列 | 内容 |
|---|---|
| 識別ID/種別 | `projects.code` + `issues.number` による識別ID（例: `AC-dev-001`）をバッジ表示＋種別名を併記。プロジェクト自体は一意なため名前は表示せずコードのみとする。カンバンビューのカードも同じ識別ID表示で一貫させる |
...
- **新規登録を一時的に停止中（実装済み）。** 検証用に必要なアカウントを作成し終えたため、第三者が自由にサインアップできる状態を閉じている。
  - サーバー側: Cloud Functions の Auth ブロッキング関数 `functions/src/blockNewSignups.ts`（`beforeUserCreated`）が、理由を問わずすべての新規アカウント作成を`permission-denied`で拒否する。既存ユーザーのサインインには影響しない
  - **Auth ブロッキング関数の利用には Google Cloud Identity Platform（GCIP）へのアップグレードが必須**（プレーンなFirebase Authenticationのままでは`beforeUserCreated`を配線できず、`OPERATION_NOT_ALLOWED`で失敗する）。本プロジェクトは対応済み（無料枠内、既存ユーザーへの影響なし）
  - クライアント側: `src/features/auth/LoginPage.tsx` の `SIGNUPS_ENABLED` フラグで「新規登録」導線自体を非表示にしている（UIを迂回されても`blockNewSignups`が最終防衛になる）
  - 再開する場合は `SIGNUPS_ENABLED` を `true` に戻し、`functions/src/index.ts` から `blockNewSignups` のexportを外してデプロイし直す
...
```
{
  id: string
  name: string
  code: string        // 識別用の短いコード（例: 'AC-dev'）。課題の識別ID表示に使う（例: AC-dev-001）
...
{
  id: string
  number: number          // プロジェクト内で一意の連番（作成時にトランザクションで自動採番、以後不変）。projects.code と組み合わせて識別ID（例: AC-dev-001）を表示する
...
| `blockNewSignups.ts` | Authブロッキング関数（`beforeUserCreated`）。新規登録を一時停止中は全件`permission-denied`で拒否（本番デプロイ済み、us-east1、GCIP必須） |
