...
2. **カンバンビュー**
   ...
   - 初期実装は表示のみとし、ドラッグ&ドロップでのステータス変更は後続追加
3. **ガントチャート（期限俯瞰ビュー）**
   - 実装コストが高く、必要性が顕在化してから検討
...
- 新規ユーザー登録時は Cloud Functions（`functions/src/onUserCreate.ts`、Authユーザー作成トリガー）が `users/{uid}` を `role: 'member'` で自動作成する。最初の管理者への昇格はFirebase Console等での手動編集に限る（自己申告的な昇格手段は意図的に用意しない）
...
| 通知 | 期限接近・ステータス変更・担当者アサイン時に Cloud Functions + FCM／メールで通知 | 中 |
...
| ガントチャート的な期限俯瞰ビュー | 複数課題の期限を時系列で俯瞰 | 低（後回し可） |

> 通知（期限接近・ステータス変更・担当者アサイン時の Cloud Functions + FCM／メール通知）は初期スコープ外とし、後続追加とする。
...
- ダッシュボードのビュー実装順：リストビュー → カンバンビュー → ガントチャート
...
| `lib/firebase.ts` | Firebase初期化、Emulator接続のopt-inスイッチ |
...
- 最初の管理者アカウントを作る仕組み（現状Firebase Console上での手動編集のみ。セルフサービスでの昇格手段は意図的に用意していない）
