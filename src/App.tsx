    ...
    <div className="mx-auto max-w-6xl p-3 sm:p-6">
      ...
      <div className="mb-4 flex flex-col gap-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h1 className="text-xl font-semibold text-slate-800 sm:text-2xl">タスク管理</h1>
          <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600">
            ...
            <span className="hidden sm:inline">{profile?.name ?? authUser.email}</span>
            <button
              onClick={() => signOutUser()}
              className="rounded-md border border-slate-300 px-2.5 py-1 text-xs text-slate-600 hover:bg-slate-50"
            >
        ...
        <nav className="-mx-3 flex gap-1 overflow-x-auto px-3 text-sm sm:mx-0 sm:flex-wrap sm:px-0">
          <button
            onClick={() => setView('issues')}
            className={`shrink-0 rounded-md px-3 py-1.5 ${view === 'issues' ? 'bg-slate-800 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            課題一覧
          </button>
          <button
            onClick={() => setView('kanban')}
            className={`shrink-0 rounded-md px-3 py-1.5 ${view === 'kanban' ? 'bg-slate-800 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            カンバン
          </button>
          <button
            onClick={() => setView('projects')}
            className={`shrink-0 rounded-md px-3 py-1.5 ${view === 'projects' ? 'bg-slate-800 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            プロジェクト管理
          </button>
          {isAdmin && (
            <button
              ...
              className={`shrink-0 rounded-md px-3 py-1.5 ${view === 'masterData' ? 'bg-slate-800 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
            >
              種別・ステータス管理
            </button>
          )}
          {isAdmin && (
            <button
              ...
              className={`shrink-0 rounded-md px-3 py-1.5 ${view === 'userManagement' ? 'bg-slate-800 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
            >
              ユーザー管理
            </button>
          )}
        ...
        <span className="text-xs text-slate-500 sm:hidden">{profile?.name ?? authUser.email}</span>
      </div>
